...
class ValidationServiceSpec extends org.specs2.Specification { def is = s2"""
  ...
  POST /validate/schema/jsonschema returns linting errors for self-describing schema $e2
  ...
  def e9 = {
    ...
    val request = Request[IO](Method.POST, Uri.uri("/validate/instance"))
      .withHeaders(Headers(Header("apikey", SpecHelpers.readKey.toString)))
      .withBodyStream(toBytes(instance))
  ...
  def e10 = {
    ...
    val expected =
      json"""{"message" : "Schema not found"}"""
    val request = Request[IO](Method.POST, Uri.uri("/validate/instance"))
      .withHeaders(Headers(Header("apikey", "00000000-1111-eeee-0000-eeeeeeeeffff")))
      .withBodyStream(toBytes(instance))
