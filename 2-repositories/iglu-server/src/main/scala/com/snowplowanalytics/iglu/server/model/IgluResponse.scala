...
object IgluResponse {
  ...
  val NotFoundSchema = "Schema not found"
  val NotAuthorized = "Not authorized"
  val Mismatch = "Schema metadata mismatches in payload and URI"
  val DecodeError = "Cannot decode JSON Schema"
  val SchemaInvalidationMessage = "The schema has some issues"
  val DataInvalidationMessage = "The instance is invalid against its schema"
  ...
  // TODO: why this is not in iglu core?
  implicit val schemaKeyServerDecoder: Decoder[SchemaKey] =
    Decoder.instance { cursor =>
      cursor.as[String]
        .map(SchemaKey.fromUri)
        .flatMap {
          case Left(e) => DecodingFailure(s"Cannot decode schema key, ${e.code}", cursor.history).asLeft
