...
class SchemaService(schemaActor: ActorRef, apiKeyActor: ActorRef)
        ...
        headerValueByName("apikey") { apikey =>
          auth(apikey) { authPair =>
            ...
            get {
              ...
              path((VendorPattern / NamePattern / FormatPattern / VersionPattern).repeat(1, Int.MaxValue, ",")) { list =>
                val (vendors, names, formats, versions) = (list.map(_._1), list.map(_._2), list.map(_._3), list.map(_._4))
                readRoute(vendors, names, formats, versions, authPair._1, authPair._2)
              } ~
              pathPrefix(VendorPattern.repeat(1, Int.MaxValue, ",")) { vendors: List[String] =>
                pathPrefix(NamePattern.repeat(1, Int.MaxValue, ",")) { names: List[String] =>
                  pathPrefix(FormatPattern.repeat(1, Int.MaxValue, ",")) { formats: List[String] =>
                    ...
                    pathEnd {
                      readFormatRoute(vendors, names, formats, authPair._1, authPair._2)
                    }
                  } ~
                  pathEnd {
                    readNameRoute(vendors, names, authPair._1, authPair._2)
                  }
                } ~
                pathEnd {
                  readVendorRoute(vendors, authPair._1, authPair._2)
                }
              }
            }
          }
  ...
  @ApiImplicitParams(Array(
    ...
    new ApiImplicitParam(name = "isPublic",
      value = "Do you want your schema to be publicly available?",
  ...
  @ApiOperation(value = "Updates or creates a schema in the repository",
    httpMethod = "PUT", consumes = "application/json")
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "vendor", value = "Schema's vendor",
      required = true, dataType = "string", paramType = "path"),
    new ApiImplicitParam(name = "name", value = "Schema's name",
   ...
   * @param permission API key's permission
   */
  @ApiOperation(value = "Delete route", hidden = true)
  ...
  def readVendorRoute(@ApiParam(hidden = true) v: List[String],
    ...
    (parameter('filter.?) | formField('filter.?)) { filter =>
      filter match {
        ...
        case _ =>
          (parameter('metadata.?) | formField('metadata.?)) { metadata =>
            metadata match {
              case Some("1") => complete {
                (schemaActor ? GetSchemasFromVendor(v, o, p, includeMetadata = true))
                  .mapTo[(StatusCode, String)]
              }
              case _ => complete {
      ...
      filter match {
        ...
        case _ =>
          (parameter('metadata.?) | formField('metadata.?)) { metadata =>
            metadata match {
              case Some("1") => complete {
                (schemaActor ? GetSchemasFromName(v, n, o, p, includeMetadata = true))
                  .mapTo[(StatusCode, String)]
              }
              case _ => complete {
  ...
  @ApiOperation(value = """Retrieves every version of a particular format of a
    schema""", notes = "Returns a collection of schemas", httpMethod = "GET", response = classOf[Schema])
  @ApiImplicitParams(Array(
    new ApiImplicitParam(name = "vendor", value = "Comma-separated list of schema vendors",
      required = true, dataType = "string", paramType = "path", allowMultiple = true),
    new ApiImplicitParam(name = "name", value = "Comma-separated list of schema names",
      ...
      filter match {
        ...
        case _ =>
          (parameter('metadata.?) | formField('metadata.?)) { metadata =>
            metadata match {
              case Some("1") => complete {
                (schemaActor ? GetSchemasFromFormat(v, n, f, o, p, includeMetadata = true))
                  .mapTo[(StatusCode, String)]
              }
              case _ => complete {
  ...
  /**
    ...
    */
  @Path("/{vendor}/{name}/{schemaFormat}/{version}")
  @ApiOperation(value = """Retrieves a schema based on its (vendor, name, format, version)""",
    notes = "Returns a schema", httpMethod = "GET", response = classOf[Schema])
  @ApiImplicitParams(Array(
      ...
      filter match {
        ...
        case _ =>
          (parameter('metadata.?) | formField('metadata.?)) { metadata =>
            metadata match {
              case Some("1") => complete {
                (schemaActor ? GetSchema(v, n, f, vs, o, p, includeMetadata = true))
                  .mapTo[(StatusCode, String)]
              }
              case _ => complete {
