...
import com.snowplowanalytics.iglu.ctl.{Common, Failing, File, Result => IgluctlResult}
...
object Pull {
  ...
  def getSchemas(request: HttpRequest): EitherT[IO, String, HttpResponse[String]] =
    EitherT(IO { Either.catchNonFatal(request.asString).leftMap(_.getMessage) }).flatMap { response =>
      if (response.code == 200) EitherT.pure[IO, String](response)
      else EitherT.leftT[IO, HttpResponse[String]]("Status code is not 200.")
    }

  def writeSchemas(schemas: String, output: Path) = {
    val parsed = jacksonParse(schemas).as[List[Json]].map(igluParse(_).getOrElse(throw new RuntimeException("Invalid self-describing JSON schema"))).map {
