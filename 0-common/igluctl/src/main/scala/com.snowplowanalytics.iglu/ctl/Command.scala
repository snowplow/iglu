      ...
      .children(
        ...
        opt[List[Linter]]("skip-checks")
          ...
          text "Lint without specified linters, given comma separated\tDefault: None\n" +
                ...
                "\t\t\t   rootType              : Check that root of schema has object type and contains properties\n" +
                "\t\t\t   unknownFormats        : Check that schema doesn't contain unknown formats\n" +
                "\t\t\t   numericMinMaxPresent  : Check that schema with numeric type contains both minimum and maximum properties\n" +
                "\t\t\t   extractStringMaxLength: Check that schema with string type contains maxLength property or other ways to extract max length\n" +
                "\t\t\t   nonRequiredFields     : Check that non-required fields have null type\n" +
                "\t\t\t   descriptionPresent    : Check that each property contains a description property\n"
      )
  }
}
