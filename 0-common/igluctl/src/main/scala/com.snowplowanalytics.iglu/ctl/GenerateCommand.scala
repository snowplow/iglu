...
case class GenerateCommand(
  ...
  private[ctl] def transformSelfDescribing(files: List[JsonFile]): DdlOutput = {
    ...
    DdlOutput(
      ...
      warnings = schemaVerWarnings.messages ++ schemaVerErrors.messages ++ schemaErrors ++ ddlErrors ++ ddlWarnings)
  }
  ...
  /**
    ...
    */
  private[ctl] def validateSchemaVersions(schemas: List[IgluSchema]): (Warnings, Errors) = {
    ...
    if (schemas.empty) {
      (Warnings(List.empty[String]), Errors(List.empty[String]))
    } else {
      if (input.isFile) {
        val schemaVerWarning = schemas.head.self.version match {
          case SchemaVer.Full(1, 0, 0) => Warnings(List.empty[String])
          case _                       => Warnings(List(s"Warning: File [${input.getAbsolutePath}] contains a schema whose version is NOT 1-0-0"))
        }
        (schemaVerWarning, Errors(List.empty[String]))
      } else {
        val schemasGroupByVendor: Map[String, List[IgluSchema]] = schemas.groupBy(_.self.vendor)

        val schemasByVendorGroupByName: Iterable[List[List[String]]] =
          for {
            (vendor, schemas) <- schemasGroupByVendor
            schemasGroupByName: Map[String, List[SchemaMap]] = schemas.map(schema => schema.self).groupBy(_.name)

            firstVersionNotFoundErrors = for {
              (name, schemas) <- schemasGroupByName
              if !schemas.exists(sm => sm.version == SchemaVer.Full(1, 0, 0)) && !force
            ...
            schemaVerGapErrors = for {
              (name, schemas) <- schemasGroupByName
              sortedSchemaMaps = schemas.sortWith(_.version.asString < _.version.asString)
          ...
          } yield List(List.empty[String], firstVersionNotFoundErrors.toList ::: schemaVerGapErrors.toList)

        val concatenatedErrors: List[String] = schemasByVendorGroupByName.toList.flatten.flatten

        (Warnings(List.empty[String]), Errors(concatenatedErrors))
      }
    }
  }
