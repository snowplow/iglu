...
// Schema DDL
import com.snowplowanalytics.iglu.schemaddl.jsonschema.SanityLinter.{ allLinters, lintUnknownFormats, lintRootType }
...
class CommandSpec extends Specification { def is = s2"""
  ...
  def e4 = {
    val lint = Command
      .cliParser
      .parse("lint . --skip-checks unknownFormats,rootType".split(" "), Command())
      .flatMap(_.toCommand)

    val skippedChecks = List(lintUnknownFormats, lintRootType)
