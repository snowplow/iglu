...
object SanityLinter {
   ...
   */
  val lintNumericMinMaxPresent: Linter = (schema: Schema) => {
   ...
   */
  val lintExtractStringMaxLength: Linter = (schema: Schema) => {
  ...
  /**
    ...
    */
  val lintNonRequiredFields: Linter = (schema: Schema) => {
    (schema.required, schema.properties) match {
      case (Some(Required(required)), Some(Properties(properties))) =>
        ...
        optKeysWithoutTypeNull.isEmpty.or("It is recommended that non-required fields have null type")
      case _ => propertySuccess
    }
  }

  /**
   * Check that each property contains a description property
   */
  ...
  val allLinters: Map[String, Linter] = Map(
    ...
    "numericMinMaxPresent"      -> lintNumericMinMaxPresent,
    "extractStringMaxLength"    -> lintExtractStringMaxLength,
    "nonRequiredFields"         -> lintNonRequiredFields,
