...
object SchemaKey {
   ...
   */
  val schemaUriRegex = "^iglu:([a-zA-Z0-9-_.]+)/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)/([0-9]+-[0-9]+-[0-9]+)$".r

  /**
   * Regular expression to extract SchemaKey from path
   */
  val schemaPathRegex = "^([a-zA-Z0-9-_.]+)/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)/([0-9]+-[0-9]+-[0-9]+)$".r
  ...
  val ordering = Ordering.by { (key: SchemaKey) =>
    (key.vendor, key.name, key.format, key.version.model, key.version.revision, key.version.addition)
  }
